...
const BenefitsTable = function(props) {
  ...
  var SNAPBenefitCurrent = curr.hasSnap
      ? Math.round(getSNAPBenefits(client, "current"))
      : 0,
    SNAPBenefitFuture = curr.hasSnap
      ? Math.round(getSNAPBenefits(client, "future"))
      : 0,
    SNAPDiff = SNAPBenefitFuture - SNAPBenefitCurrent,
    sec8BenefitCurrent = curr.hasHousing
      ? Math.round(getHousingBenefit(client, "current"))
      : 0,
    sec8BenefitFuture = curr.hasHousing
      ? Math.round(getHousingBenefit(client, "future"))
      : 0,
    ...
    return (
      <Table.Row>
        ...
        <Table.Cell textAlign="right">
          {getSignSymbol(SNAPDiff)} ${Math.abs(SNAPDiff)} / month
        </Table.Cell>
      </Table.Row>
    );
  };
  ...
  const Sec8BenefitRow = function(props) {
    ...
    return (
      <Table.Row>
        ...
        <Table.Cell textAlign="right">
          {getSignSymbol(sec8Diff)} ${Math.abs(sec8Diff)} / month
        </Table.Cell>
      </Table.Row>
    );
  };
  ...
  const TotalBenefitsRow = function(props) {
    ...
    return (
      <Table.Row>
        <Table.Cell textAlign="right" width={3} style={totalsRowHeaderStyle}>
          Total Benefits
        </Table.Cell>
        <Table.Cell textAlign="right" width={3} style={totalsRowStyle}>
          ${totalBenefitCurrent} / month
        </Table.Cell>
        <Table.Cell textAlign="right" width={3} style={totalsRowStyle}>
          ${totalBenefitFuture} / month
        </Table.Cell>
        <Table.Cell textAlign="right" width={3} style={totalsRowStyle}>
          {getSignSymbol(totalDiff)} ${Math.abs(totalDiff)} / month
        </Table.Cell>
      </Table.Row>
    );
  };
  ...
  const IncomeRow = function(props) {
    return (
      <Table.Row>
        ...
        <Table.Cell textAlign="right">
          {getSignSymbol(incomeDiff)} ${Math.abs(incomeDiff)} / month
        </Table.Cell>
      </Table.Row>
    );
  };
  ...
  const TotalsRow = function(props) {
    return (
      <Table.Row style={{ border: "none" }}>
        ...
        <Table.Cell textAlign="right" width={3} style={totalsRowStyle}>
          {getSignSymbol(netDiff)} ${Math.abs(netDiff)} / month
        </Table.Cell>
      </Table.Row>
    );
  };
  ...
  return (
    <div>
      <Table celled>
        <Table.Header>
          <Table.Row>
            <Table.Cell style={columnHeaderStyle} width={3}>
              Benefit
            </Table.Cell>
            <Table.Cell style={columnHeaderStyle} width={3}>
              Current Benefits
