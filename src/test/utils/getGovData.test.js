...
*/
const testYearlyLimit = function(
  testname,
  pguidelines,
  pitems,
  ppercent,
  ...
  test(
    ...
    () => {
      if (typeof ppercent == stringString && ppercent == undefinedString) {
        expect(getgovdata.getYearlyLimitBySize(pguidelines, pitems)).toBe(
          expected
        );
      } else {
        expect(
          getgovdata.getYearlyLimitBySize(pguidelines, pitems, ppercent)
        ).toBe(expected);
      }
    }
  );
...
testYearlyLimit(
  "3. Test when percent=200 doubles",
  fedPovertyGuidelines,
  2,
  200,
  32480
...
testYearlyLimit(
  "5. Test with items out of range",
  fedPovertyGuidelines,
  5,
  undefinedString,
  16240 + 3 * 4180
...
testYearlyLimit(
  "6. Test of function for each additional",
  fedPovertyGuidelines,
  5,
  undefinedString,
  20000 + 3 * 100
