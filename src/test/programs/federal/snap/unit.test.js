...
describe("SNAPhelpers", () => {
  ...
  describe("`.getAdjustedGross( timeClient )` given a time-restricted client object", () => {
    ...
    it("with positive raw adjusted gross income, should return that income", () => {
      current.earned = 1000;
      const income =
        current.earned + grossUnearnedIncome - current.childSupportPaidOut;
  ...
  test("`getPovertyGrossIncomeLevel( timeClient )", () => {
    ...
    const getMonthlyLimitBySize = jest.spyOn(
      getGovData,
    ...
    expect(SNAPhelpers.getPovertyGrossIncomeLevel(current)).toEqual(
      monthlyLimit
    );
    expect(getMonthlyLimitBySize).toBeCalledWith(
  ...
  describe("`.getDependentCareDeduction( timeClient ) given a time-restricted client object with", () => {
    ...
    it("under 13 dependents, it should return related expenses", () => {
      ...
      expect(SNAPhelpers.getDependentCareDeduction(current)).toEqual(
        UNDER13_CARE_EXPENSES.length
      );
    });
  ...
  test("`.getHalfAdjustedIncome( timeClient )", () => {
    ...
    const getAdjustedNotGrossIncome = jest.spyOn(
      SNAPhelpers,
  ...
  describe("`.getShelterDeduction( timeClient )` given a time-restricted client object with", () => {
    ...
    beforeEach(() => {
      current = cloneDeep(defaultCurrent);
      getRawShelterDeduction = jest.spyOn(
        SNAPhelpers,
    ...
    it("a raw deduction above the cap, it should return the cap", () => {
      const rawDeduction = SNAPData.SHELTER_DEDUCTION_CAP + 1;
      getRawShelterDeduction.mockReturnValue(rawDeduction);
      expect(SNAPhelpers.getShelterDeduction(current)).toEqual(
        SNAPData.SHELTER_DEDUCTION_CAP
      );
    });
  });
  ...
  describe("`.getHomelessDeduction() given a time-restriced client object with", () => {
    ...
    it("homeless, it should return the homeless deduction", () => {
      current.shelter = "homeless";
      expect(SNAPhelpers.getHomelessDeduction(current)).toEqual(
        SNAPData.HOMELESS_DEDUCTION
      );
    });
  ...
  describe("`.getAdjustedNotGrossIncome( timeClient )` given a time-restricted client object with", () => {
    ...
    it("positive income after deductions, it should return that adjusted income", () => {
      ...
      mocks = [
        jest
          .spyOn(SNAPhelpers, "getAdjustedGross")
          .mockReturnValue(adjustedGross),
        jest
          .spyOn(SNAPhelpers, "getStandardDeduction")
          .mockReturnValue(standardDeduction),
        jest
          .spyOn(SNAPhelpers, "getEarnedIncomeDeduction")
          .mockReturnValue(earnedIncomeDeduction),
        jest
          .spyOn(SNAPhelpers, "getMedicalDeduction")
          .mockReturnValue(medicalDeduction),
        jest
          .spyOn(SNAPhelpers, "getDependentCareDeduction")
          .mockReturnValue(dependentCareDeduction)
      ];
  ...
  describe("`.monthlyNetIncome( timeClient )` given a time-restricted client object with", () => {
    ...
    it("positive income after deductions, it should return that adjusted income", () => {
      ...
      mocks = [
        jest
          .spyOn(SNAPhelpers, "getAdjustedNotGrossIncome")
          .mockReturnValue(income),
        jest
          .spyOn(SNAPhelpers, "getHomelessDeduction")
          .mockReturnValue(homelessDeduction),
        jest
          .spyOn(SNAPhelpers, "getShelterDeduction")
          .mockReturnValue(shelterDeduction)
      ];
  ...
  describe("`.getMaxNetIncome( timeClient )` given a time-restricted client object with", () => {
    ...
    beforeEach(() => {
      current = cloneDeep(defaultCurrent);
      getPovertyGrossIncomeLevel = jest.spyOn(
        SNAPhelpers,
    ...
    it("income above poverty line and disabled members, it should return the yearly limit", () => {
      ...
      const getYearlyLimitBySize = jest.spyOn(
        getGovData,
