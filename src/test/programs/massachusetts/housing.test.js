import {
  getHousingBenefit,
  section8Helpers
} from "../../../programs/massachusetts/housing";
...
describe("section8Helpers", () => {
  ...
  describe("`.getNetIncome`", () => {
    it("should return net income", () => {
      expect(
        section8Helpers.getNetIncome(sampleClients.row7, "current")
      ).toEqual(2755);
    });
  });
  ...
  describe("`.getAdjustedIncome`", () => {
    it("should return adjusted income if no member is elderly or disabled", () => {
      ...
      expect(
        section8Helpers.getAdjustedIncome(sampleClients.row3, "current", net)
      ).toEqual(677);
    });
    ...
    it("should return adjusted income with additional allowances if a member is elderly or disabled", () => {
      ...
      expect(
        section8Helpers.getAdjustedIncome(sampleClients.row4, "current", net)
      ).toEqual(974 + 2 / 3);
    });
  });
  ...
  describe("`.getDisabledAndMedicalAllowancesSum( timeClient )`", () => {
    ...
    it("should return hcapMin if the head or spouse in not disabled or elderly", () => {
      ...
      let assSubstracted =
        sampleClients.row8["current"].disabledAssistance - net * 0.03;
      let hcapAllowance = Math.min(
        assSubstracted,
        sampleClients.row8["current"].earnedBecauseOfAdultCare
      );
  ...
  describe("`.getMedicalExpenses( timeClient )`", () => {
    it("should return the medical expenses", () => {
      expect(
        section8Helpers.getMedicalExpenses(sampleClients.row3, "current")
      ).toEqual(0);
    });
  });
  ...
  describe("`.isDisabledOrElderly( timeClient )` given a time-restricted client object with", () => {
    ...
    it("no disabled or elderly member should return false", () => {
      expect(
        section8Helpers.isDisabledOrElderly(
          sampleClients.row4.current.household[1]
        )
      ).toEqual(false);
    });
    ...
    it("a disabled member but no elderly member should return true", () => {
      expect(
        section8Helpers.isDisabledOrElderly(
          sampleClients.row4.current.household[0]
        )
      ).toEqual(true);
    });
    ...
    it("no disabled member but an elderly member should return true", () => {
      ...
      expect(section8Helpers.isDisabledOrElderly(current.household[0])).toEqual(
        true
      );
    });
    ...
    describe("`.hasAnyDsbOrElderly` given a disabled and elderly member", () => {
      it("should be greater than zero", () => {
        expect(
          section8Helpers.hasAnyDsbOrElderly(defaultClient, "current")
        ).toEqual(true);
      });
    });
    ...
    describe("`.hasDsbOrEldHeadOrSpouse` given a disabled and elderly head", () => {
      it("should be greater than zero", () => {
        expect(
          section8Helpers.hasDsbOrEldHeadOrSpouse(defaultClient, "current")
        ).toEqual(true);
      });
    });
  });
