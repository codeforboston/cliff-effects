...
// LOGIC/UTILITIES
import { sumProps, getGrossUnearnedIncomeMonthly } from "../../utils/cashflow";
import {
...
const getSNAPBenefits = function(client, timeframe) {
  ...
  var finalResult = 0,
    ...
    maxSnapAllotment = getYearlyLimitBySize(
      SNAPData.SNAP_LIMITS,
      hlp.householdSize(client)
    ),
    thirtyPercentNetIncome =
      hlp.monthlyNetIncome(client) * SNAPData.PERCENT_OF_NET,
...
hlp.getAdjustedGross = function(client) {
  var raw =
    client.earned +
    getGrossUnearnedIncomeMonthly(client) -
    client.childSupportPaidOut;
  return Math.max(0, raw);
};

hlp.getPovertyGrossIncomeLevel = function(client) {
  return getMonthlyLimitBySize(
...
hlp.getNonUtilityCosts = function(client) {
  ...
  if (hlp.isHomeless(client)) {
    ...
    shelterCost =
      client.mortgage + client.housingInsurance + client.propertyTax;
...
hlp.getStandardDeduction = function(client) {
  return getYearlyLimitBySize(
    SNAPData.STANDARD_DEDUCTIONS,
    hlp.householdSize(client)
  );
};
...
hlp.getMedicalDeduction = function(client) {
  ...
  if (hlp.hasDisabledOrElderlyMember(client) === true) {
    ...
    if (
      medicalExpenses >= SNAPData.MIN_MEDICAL_EXPENSES &&
      medicalExpenses <= SNAPData.MAX_MEDICAL_EXPENSES
    ) {
...
hlp.getHalfAdjustedIncome = function(client) {
  return hlp.getAdjustedNotGrossIncome(client) * 0.5;
};
