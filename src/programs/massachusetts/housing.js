...
hlp.getDisabledAndMedicalAllowancesSum = function(client, timeframe, net) {
  ...
  var handcpExpense = client[time].disabledAssistance, // #7
    ...
    handicapAllowance = hlp.getMinHandicapAllowance(
      client,
      time,
      assistanceRemainder
    ); // #10, #11
...
hlp.hasAnyDsbOrElderly = function(client, timeframe) {
  var numMatches = getEveryMemberOfHousehold(
    client[timeframe],
    hlp.isDisabledOrElderly
  ).length;
  return numMatches > 0;
...
hlp.hasDsbOrEldHeadOrSpouse = function(client, timeframe) {
  ...
  var numMatches = getEveryMemberOfHousehold(client[timeframe], comboTest)
    .length;

  return numMatches > 0;
}; // End hlp.hasDsbOrEldHeadOrSpouse()

export { getHousingBenefit, section8Helpers };
